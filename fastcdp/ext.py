...
# %% auto #0
__all__ = ['ExtChannel', 'ExtCDP', 'ExtPage', 'ExtTargets', 'cdp_yolo']
...
class ExtTargets(list):
    ...
    def __repr__(self):
        return '\n'.join(f"{t.tabId} {'*' if t.get('attached') else ' '} {truncstr(t.get('title') or '', 40)} {(t.get('url') or '')[:70]}" for t in self)

@patch(as_prop=True)
async def pages(self:ExtCDP)->ExtTargets: # Rows with `tabId`, `id`, `title`, `url`, ...; `*` marks debugger-attached
    "The browser's open tabs"
    return ExtTargets(AttrDict(t) for t in await self._action('get-targets') if t.get('type')=='page')
...
def cdp_yolo():
    ...
    allow({ExtChannel:..., ExtCDP:..., ExtPage:..., ExtTargets:...})
