...
Choose click semantics deliberately. `click(backend_id)` sends real pointer movement, press, and release events, which UI smoke tests and hover/mousedown behavior need. `dom_click(backend_id)` calls the element's DOM `click()` method; use it when pointer events are unnecessary or a protected target does not acknowledge CDP mouse input (Google's unverified-app warning is one example). Both bound the whole action to five seconds. Never retry a timed-out click automatically: inspect the resulting page first, because the action may already have happened.
...
Waiting is built in -- never `sleep` and re-read. `goto` waits for `load` by default; pass `wait='idle'` only when initial requests must settle, or `wait=None` before an application-specific content wait. `click_and_wait` sends real pointer events, requires a top-frame navigation, and uses the same wait modes. Wrap any other navigation action in `expect_navigation`; never try to wait for navigation after the action, because its events may already be gone. For content that changes *in place* (tab panels, htmx swaps, SPAs), use `click`/`dom_click` and then `wait_for_ax(role, name)`, `wait_for_text`, `wait_for_selector`, or `wait_for(js_expr)`. `set_content(html)` replaces the document wholesale -- the way to put fixture HTML in a page, since `data:` URLs can't be navigated to on the extension path (`goto` raises `net::ERR_ABORTED`).
